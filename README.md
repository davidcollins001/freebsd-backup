...
This repo contains my automated setup for Freebsd system backups.
It's designed solely to meet my own backup requirements (i.e., not to be
general purpose):

1. Generate a single archive with a backup of config data on a configurable
   schedule.

2. Retain backup archives on the local host for a configurable number of days.
...
Note: This single-archive backup scheme isn't space-efficient if your system
includes large file, as they will be re-uploaded with each backup. If this is an
issue, you might consider modifying the script to use
[restic](https://restic.net/) instead.

## Prerequisites

1. A [cron](https://en.wikipedia.org/wiki/Cron) daemon. This is used to run
   backup actions on a scheduled basis.

2. Optionally, a `gpg` (GnuPG 2.x) binary (https://gnupg.org/). This can be
   installed via the `gnupg` package on Debian/Ubuntu or the `gnupg2` package
   on RHEL/CentOS/Fedora.

3. Optionally, an `rsync` binary

## Usage

1. Start by cloning this repo to the server
...
2. Copy the `backup.conf.template` file to `backup.conf`.
   ...
   2. Note that `backup.conf` is simply sourced into the `backup.sh` script, so
...
3. Modify the `backup/crontab.template` file as needed. This crontab actually
   ...
   1. Review the backup schedule. I generate backup archives hourly, but you
      might prefer to do this less frequently to save space.

   2. Review the local backup archive retention policy. I delete archives
      older than 14 days (`-mtime +14`). Adjust this if needed.

   3. Review the log file retention policy. I delete log files older than
      14 days (`-mtime +14`). Adjust this if needed.

4. Install the crontab under a user (typically your normal login) that can
   read your backup data. In many cases, running `crontab -e` and pasting the
   contents of the filled-in crontab template file should work. Note that if
   your cron user doesn't have write permissions to the database, then you must
   ensure it has write permissions to the backup data directory,
...
$HOME/backup
├── archives
│   ├── system-host-20210101-0000.tar.xz
│   ├── system-host-20210101-0000.tar.xz.gpg
│   ├── system-host-20210101-0100.tar.xz
│   ├── system-host-20210101-0100.tar.xz.gpg
│   └── ...
├── backup.conf
